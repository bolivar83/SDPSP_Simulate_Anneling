function [FO,M,BestSchedule] = SA00(DataProject,RRHH,TA,CP,W,CI,To,Alpha)
...
[SolInit,FVo,NP,CA] = SSGSpdNew(DataProject,RRHH,TA,CP,W);
...
FO=zeros(1,CI);
 ...
 while i < CI 
      ...
      [Schedule]=NeighborhoodAT(DataProject,SolInit,NP,CA);
...
function [Schedule,FO,NP,CA] = SSGSpdNew(DataProject,RRHH,TA,CP,W)
% SSGSpd Summary of this function [Schedule,FO,CA,NP] = SSGSpdNew(DataProject,RRHH,TA,CP,W)
% Autor: Lic Bolivar E. Medrano Broche. CEGEL & Dpto. Ciencias Básicas.
% Facultad III. Universidad de las Ciencias Informáticas
...
Schedule=cat(3,zeros(NP,CA),zeros(NP,CA),zeros(NP,CA),W); % Estructura de la solución
...
function [Schedule]=NeighborhoodAT(DataProject,Schedule,NP,CT)
% NeighborhoodA function  
% Autor: Lic Bolivar E. Medrano Broche. CEGEL & Dpto. Ciencias Básicas.
% Facultad III. Universidad de las Ciencias Informáticas
% Descripción: Reasigna prioridades a todas las actividades de Tipo  k  de
% un proyecto elegido aleatoriamente  
% actividades del  mismo tipo y del mismo proyecto.
% ENTRADAS
% DataProject -> datos de lso proyectos 
% Schedule -> Solución actual 
% Numero de proyectos -> NP,
% Numero de actividades de cada proyecto -> CA
% SALIDAS
% Schedule -> Solución Modificada

% IMPLEMENTACIÓN 

SelectTipoA=unidrnd(CT); % la seleción del proyecto tambien puede ser considereada alguna regla

SelectProject1=unidrnd(NP);

 Pro=DataProject(:,:,SelectProject1);
 
 Pro(Pro(:,2)==SelectTipoA,1);
  
 ActSelectTipoA=Pro(Pro(:,2)==SelectTipoA,1);
 
Schedule(SelectProject1,ActSelectTipoA,4)=rand(1,length(ActSelectTipoA)); % La prioridades generadas aleatoriamente pueden ser intercambiadas por prioridades previamente generadas por por una regla de prioridad dada. 
