function [FO,M,BestSchedule] = SA01(DataProject,RRHH,TA,CP,CI,To,Alpha)
% SimulateAnnealingA function Exp SA2 O(3n^4) + O(n^4) =O(4n^4)
...
FO=zeros(1,CI);
 ...
 while i < CI 
      ...
      [Schedule]=NeighborhoodA(DataProject,SolInit,NP,CA);
     
      [Schedule,FV]=SSGSSApdA(Schedule,DataProject,RRHH,TA,CP,NP,CA);
...
function [Schedule]=NeighborhoodA(DataProject,Schedule,NP,CA)
% NeighborhoodA function  
% Autor: Lic Bolivar E. Medrano Broche. CEGEL & Dpto. Ciencias Básicas.
% Facultad III. Universidad de las Ciencias Informáticas
% Descripción: Realiza una intercambio de las prioridades, de dos
...
return
     ...
     TipoA=DataProject(SelectAct1,2,SelectProject); % Tipo de actividad = tipo de recurso que requiere en la primera selección
    
     [CATipoA2,u]=find(DataProject(:,2,SelectProject)==TipoA); % conjunto de actividades de TipoA en la Segunda selección
   
     SelectAct2=CATipoA2(unidrnd(length(CATipoA2)));
       
    if  SelectAct1 ~= SelectAct2
        
        WAct2=Schedule(SelectProject,SelectAct2,4);
        
        aux=Schedule(SelectProject,SelectAct1,4);
        
        Schedule(SelectProject,SelectAct1,4)=WAct2;
        
        Schedule(SelectProject,SelectAct2,4)=aux;
...
function [Schedule,FO] = SSGSSApdA(Schedule,DataProject,RRHH,TA,CP,NP,CA)
